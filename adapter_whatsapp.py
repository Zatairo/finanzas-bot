...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
                    ...
                    if resp.status == 200:
                        ...
                        for msg_data in messages:
                            ...
                            if event:
                                ...
                                if await self._maybe_handle_expense_direct(msg_data, event):
                                    continue
    ...
    # ── Registro directo de gastos/finanzas (sin LLM, sin rate-limit) ──
    _EXPENSE_GROUPS = {
        "120363429174326751": "andrea",
        "120363426559924341": "personal",
        "120363426158712224": "hogar",
    }
    _GASTO_LIKE = (
        "pague", "pagué", "gaste", "gasté", "gasto", "pago", "pagar", "compré", "compre",
        ...
        "ultima entrada", "ultimo gasto", "deshaz",
        "ayuda", "menu", "menú", "instrucciones", "como usar", "como se usa",
        "como funciona", "que sabes hacer", "revisar", "revision", "revisión",
        "cola de aprendizaje", "pendientes de aprender", "que me falta aprender",
    )

    _QUERY_WORDS = ("gastos de", "gastos del", "resumen", "cuanto gast", "cuanto gasté",
                    "cuánto gasté", "cuanto se gast", "total de gastos", "que gastos",
                    "los gastos de", "mis gastos", "gastos de agosto", "gastos del mes",
                    "cuanto gastamos", "cuánto gastamos", "cuanto llevamos")
    _QUERY_MESES = ("enero", "febrero", "marzo", "abril", "mayo", "junio", "julio",
                    "agosto", "septiembre", "setiembre", "octubre", "noviembre", "diciembre",
                    "mes pasado", "mes anterior", "este mes", "el mes", "del mes", "ultimo mes",
                    "meses", "mes")
    _QUERY_EXTRA = ("presupuesto", "presupuestos", "frecuencia", "cada cuanto", "cada cuánto",
                    "inventario", "que tan seguido", "quien gasto", "cuanto se gasta",
                    "que compro", "en que me gasto", "donde se fue")
    ...
    def _looks_like_query(self, text: str) -> bool:
        ...
        if re.search(r"(^|\s)\d[\d.,]{4,}(\s|$)", t):
            return False
        if re.search(r"\b(pague|pagué|compre|compré|recargue|llevé|abone|aboné)\b", t):
            return False
        if any(w in t for w in self._QUERY_WORDS):
            return True
        if any(re.search(r"\b" + re.escape(w) + r"\b", t) for w in self._QUERY_MESES):
            return True
        if any(w in t for w in self._QUERY_EXTRA):
            return True
        return False
    ...
    async def _maybe_handle_expense_direct(self, msg_data, event) -> bool:
        try:
            ...
            print("[whatsapp] expense-check chat=%r key=%r body=%r type=%r" % (chat_id, self._EXPENSE_GROUPS.get(chat_id.split('@')[0]), str(msg_data.get('body', '') or '')[:80], str(getattr(event, 'message_type', None))), flush=True)
            local = chat_id.split("@")[0]
            key = self._EXPENSE_GROUPS.get(local)
            if not key:
                return False
            body = str(msg_data.get("body", "") or "")
            is_photo = event.message_type == MessageType.PHOTO
            is_pend = bool(key) and os.path.exists(os.path.join("/tmp", "gasto_pendiente_%s.json" % key))
            ...
            print("[whatsapp] gasto script=%r exists=%s" % (script, os.path.exists(script)), flush=True)
            if not os.path.exists(script):
                return False
            cmd = [_sys.executable, script, "--grupo", key]
            if is_photo and getattr(event, "media_urls", None):
                img = event.media_urls[0]
                cmd += ["--imagen", img, "--evidencia", os.path.basename(img)]
            if body:
                cmd += ["--texto", body]
            sender = str(msg_data.get("senderId", "") or "")
            if sender:
                cmd += ["--sender", sender]
            print("[whatsapp] LANZANDO gasto.py grupo=%s script=%s" % (key, script), flush=True)
            ...
            try:
                ...
                proc.kill()
                out = (b"Tiempo de espera agotado procesando el gasto.").encode()
            reply = (out or b"").decode("utf-8", errors="replace").strip()
            print("[whatsapp] gasto.py OUT (grupo=%s): %s" % (key, (out or b"").decode("utf-8", "replace")[:400]), flush=True)
            if reply:
                await self.send(chat_id, reply, reply_to=str(msg_data.get("id", "") or ""))
            return True
        except Exception as exc:
            import traceback as _tb
            print("[whatsapp] gasto direct EXCEPTION: %s\n%s" % (exc, _tb.format_exc()), flush=True)
            return False
